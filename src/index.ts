...
async function listTasks(input?: string) {
    try {
        ...
        if (input !== "" && input !== undefined) {
            const statusMap: { [key: string]: "done" | "not done" | "in progress" } = {
                D: "done",
                ND: "not done",
                IP: "in progress",
            }
            ...
            tasks.forEach((task: { status: "done" | "not done" | "in progress" }) => {
...
async function addTask(task: string) {
    try {
        ...
        const tasksQuantity = tasks.length !== 0 ? Math.max(...tasks.map((task: Task) => task.id)) + 1 : 1;
        ...
        await fs.promises.writeFile(FILEPATH, JSON.stringify(tasks, null, 4), "utf-8");
...
async function removeTask(taskNumber: number) {
    try {
        let tasks: Task[] = [];
        try {
            const fileContent = await fs.promises.readFile(FILEPATH, "utf-8");
        ...
        const taskIndex = tasks.findIndex((task: Task) => task.id === Number(taskNumber));
        if (taskIndex === -1) {
...
async function markTask(input: string) {
    try {
        let tasks: Task[] = [];
